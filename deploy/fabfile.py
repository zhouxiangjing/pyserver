...
env.user = 'root'
env.sudo_user = 'root'
env.host_string = '149.129.32.112'
env.password = ''
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop pyserver') # supervisor重启app
        sudo('supervisorctl start pyserver')
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-pyserver-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick pyserver > %s' % (db_user, db_password, f))
...
def restore2local():
    ...
    sqls = [
        'drop database if exists pyserver;',
        'create database pyserver;',
        'alter database pyserver default character set utf8 collate utf8_general_ci;' # 修改为utf8字符集
        'grant select, insert, update, delete on pyserver.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
    ...
    with lcd('backup'):
        ...
        local(r'mysql -uroot -p%s --default-character-set=utf8 pyserver < %s' % (p, restore_file[:-7])) # 导入数据库
